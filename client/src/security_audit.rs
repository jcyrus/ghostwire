...
pub enum SecurityEvent {
    ...
    /// Replay attack detected (v0.4.0)
    ReplayDetected { sender: String, nonce: String },
}
...
fn format_event(event: &SecurityEvent) -> String {
    match event {
        SecurityEvent::SessionEstablished {
            ...
            format!(
                "E2EE_SESSION_ESTABLISHED: peer={}, fingerprint={}",
        ...
        SecurityEvent::MessageEncrypted {
            recipient,
            message_id,
        } => {
            format!(
                "MESSAGE_ENCRYPTED: recipient={}, msg_id={}",
                recipient, message_id
            )
        }
        SecurityEvent::MessageDecrypted { sender, message_id } => {
            format!(
                "MESSAGE_DECRYPTED: sender={}, msg_id={}",
                sender, message_id
            )
        }
        ...
        SecurityEvent::IdentityVerified {
            peer,
            safety_number,
        } => {
            format!(
                "IDENTITY_VERIFIED: peer={}, safety_number={}",
                peer, safety_number
            )
        }
        SecurityEvent::IdentityVerificationFailed { peer, reason } => {
            format!(
                "IDENTITY_VERIFICATION_FAILED: peer={}, reason={}",
                peer, reason
            )
        }
        ...
        SecurityEvent::ReplayDetected { sender, nonce } => {
            format!("REPLAY_DETECTED: sender={}, nonce={}", sender, nonce)
        }
    }
}
